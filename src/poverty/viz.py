...
def _sample_df(df: pd.DataFrame, n: int = 12_000, seed: int = 42) -> pd.DataFrame:
    if len(df) <= n:
        return df
...
def make_oof_predictions_elasticnet(
    ...
    *,
    ...
    tag = cfg.tag(weight_mode)
    return OOFResult(df=out, model_name=model_name, tag=tag)
...
def report_metrics(paths: DataPaths, oof: pd.DataFrame) -> pd.DataFrame:
    ...
    for s in scores:
        rows.append(
            {
                "survey_id": int(s.survey_id),
                "poverty_wmape": float(s.poverty_wmape),
                "household_mape": float(s.household_mape),
                "blended": float(s.blended),
            }
        )
    rows.append(
        {
...
def plot_consumption_pred_vs_true_grid(
    ...
    *,
    ...
    y_all = np.concatenate([oof["cons_ppp17"].to_numpy(float), oof["y_pred"].to_numpy(float)])
    y_all = y_all[np.isfinite(y_all) & (y_all > 0)]
    lo = max(float(np.quantile(y_all, 0.005)), 1e-6)
    ...
    for ax, sid in zip(axes, surveys):
        ...
        y_true = gg["cons_ppp17"].to_numpy(float)
        y_pred = gg["y_pred"].to_numpy(float)
...
def plot_error_ratio_hist_grid(oof: pd.DataFrame, out_dir: Path, *, model_name: str) -> None:
    ...
    for ax, sid in zip(axes, surveys):
        g = oof[oof["survey_id"] == sid]
        y_true = g["cons_ppp17"].to_numpy(float)
...
def cmd_oof(args: argparse.Namespace) -> None:
    ...
    if args.model == "elasticnet":
        cfg = ElasticNetConfig(alpha=args.alpha, l1_ratio=args.l1_ratio, max_iter=args.max_iter, random_state=42)
        res = make_oof_predictions_elasticnet(train_df, weight_mode=weight_mode, cfg=cfg)
    elif args.model == "hgb":
        max_depth = None if args.hgb_max_depth in (0, -1) else int(args.hgb_max_depth)
        cfg = HGBConfig(
        ...
        res = make_oof_predictions_hgb(train_df, weight_mode=weight_mode, cfg=cfg)
    else:
...
def main() -> None:
    ...
    p_oof.add_argument("--model", type=str, required=True, choices=["elasticnet", "hgb"])
    ...
    p_oof.add_argument("--max-iter", type=int, default=5000)

    p_oof.add_argument("--hgb-learning-rate", type=float, default=0.05)
    p_oof.add_argument("--hgb-max-iter", type=int, default=600)
    p_oof.add_argument("--hgb-max-depth", type=int, default=6, help="Use 0 or -1 for None (no depth limit).")
    ...
    p_oof.add_argument("--hgb-early-stopping", action="store_true", default=False)
    p_oof.set_defaults(func=cmd_oof)

    p_m = sub.add_parser("metrics", help="Compute metrics report (per-survey + MEAN) from an OOF file.")
    p_m.add_argument("--data-dir", type=str, required=True)
    p_m.add_argument("--oof", type=str, required=True)
    p_m.add_argument("--out", type=str, required=True)
    p_m.set_defaults(func=cmd_metrics)

    p_plots = sub.add_parser("plots", help="Create plots + metrics_report.csv from an OOF file.")
    ...
    p_cmp = sub.add_parser("compare", help="Compare multiple OOF files (metrics + overlay plot).")
